          ...
          <el-table :data="item.user_answer" stripe>
            ...
            >
              <template slot-scope="scope">
                <span :class="scope.row.student_answer[0] === key ? 'choise' : ''">{{ `${key}:${scope.row.choice_detail[key]}` }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="result"
      ...
      <div v-if="JSON.stringify(multiple) !== '{}' && multiple.questions.length" class="question-type">
        ...
        >
          ...
          <el-table :data="item.user_answer" stripe>
            ...
            >
              <template slot-scope="scope">
                <span :class="isChoise(scope.row.student_answer, key) ? 'choise' : ''">{{ `${key}:${scope.row.choice_detail[key]}` }}</span>
              </template>
            </el-table-column>
            <el-table-column
              prop="result"
