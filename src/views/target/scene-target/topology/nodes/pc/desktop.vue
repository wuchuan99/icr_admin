<template>
  <div class="graph_node desktop">
    <i class="iconfont icon-taishiji" :style="data.styleObject" />
    <div v-if="!isInGraph && !dragging" class="graph_node_label">台式机</div>
    <div
      v-if="isInGraph"
      class="node_label"
    >{{ data.attrs.name }}</div>
...
const DesktopComputer = {
  ...
  data() {
    return {
      data: cloneDeep(DesktopComputerData)
    }
  },
  created() {
    ...
    onRetrieved(status, uuid) {
      ...
      networkElementTerminalRetrieve({ data }).then((res) => {
        if (res.status === 0) {
          ...
          // for (let i = 0; i < this.data.attrs.ip_list.length; i++) {
          //   this.data.attrs.ip_list.splice(i, 1)
          // }
          // const ip_list = res?.data?.data?.attrs?.ip_list || []
          // for (const ip of ip_list) {
          //   this.data.attrs.ip_list.push(ip)
          // }
        }
      })
    },
...
export function createDesktopComputer() {
  return { shape: 'vue-shape', component: 'desktop', data: Object(DesktopComputerData) }
}

// register component https://x6.antv.vision/zh/docs/tutorial/advanced/react#%E6%B8%B2%E6%9F%93-vue-%E8%8A%82%E7%82%B9
Graph.registerVueComponent('desktop', DesktopComputer, true)
