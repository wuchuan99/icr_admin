<template>
  <!-- <IcrCollapse title="位置和尺寸">
    <div class="position-size">
      <el-input v-model="position.x" type='number' ><span slot='prefix'>X</span></el-input>
      <el-input v-model="position.y" type='number'><span slot='prefix'>Y</span></el-input>
      <el-input v-model="size.width" type='number'><span slot='prefix'>W</span></el-input>
      <el-input v-model="size.height" type='number'><span slot='prefix'>H</span></el-input>
    </div>
  </IcrCollapse> -->
  <div class="style-container">
    <div class="icrCollapse_item">
      <el-input v-model.trim="config.name" size="mini" />
      <span class="item-title">位置和尺寸</span>
      <el-row :gutter="20" class="item-content">
        <el-col :span="12">
          <el-input v-model="position.x" size="mini"><span slot="prefix">X</span></el-input>
        </el-col>
        <el-col :span="12">
          <el-input v-model="position.y" size="mini"><span slot="prefix">Y</span></el-input>
        </el-col>
      </el-row>
      <el-row :gutter="20" class="item-content">
        <el-col :span="12">
          <el-input v-model="size.width" size="mini" @change="changeWidth"><span slot="prefix">W</span></el-input>
        </el-col>
        <span class="relevance" @click="data.relevanceFlag = !data.relevanceFlag"><i v-if="data.relevanceFlag" class="iconfont icon-guanlian" /><i v-else class="iconfont icon-quxiaoguanlian" /></span>
        <el-col :span="12">
          <el-input v-model="size.height" size="mini" @change="changeHeight"><span slot="prefix">H</span></el-input>
        </el-col>
      </el-row>
    </div>
    <div class="icrCollapse_item">
      <span v-show="node.component === 'RectBlock'" class="item-title" style="display:inline-block">边框颜色</span>
      <span v-show="node.component === 'TextBlock'" class="item-title" style="display:inline-block">文字颜色</span>
      <el-color-picker v-model="styleObject.color" popper-class="noClear" :predefine="predefineColors" style="vertical-align:middle; margin-left:10px" />
    </div>
  </div>
</template>
