<template>
  <div>
    ...
    <div class="operation-box">
      ...
      <div class="operation">
        <el-button
          ...
          @click="AddUser"
        >
          {{ `添加${ participateType === 'player' ? '选手' : '裁判'}` }}
        </el-button>
      </div>
    ...
    <el-dialog
      ...
      :title="`添加${ participateType === 'player' ? '选手' : '裁判'}`"
      @close="closeDialog"
    >
      <user-dialog
        v-if="showTableDialog"
        ref="personneluserDialog"
        :participate-type="participateType"
    ...
    <icr-confirm-delete
      ref="confirmDelete"
      title="删除参与人员"
      :desc="`您即将删除该${ participateType === 'player' ? '选手' : '裁判'}, 确定删除?`"
    />
  </div>
</template>
...
import userDialog from '../component/user_dialog.vue'
import IcrConfirmDelete from '@/components/confirm-delete.vue'
import { drillUserPaging, drillUserAdd, drillUserDelete } from '@/api/drill'
...
export default {
  ...
  setup() {
    ...
    return {
      query,
      selection,
      ...usePagination(drillUserPaging, query)
    }
  },
  props: {
    drillData: {
